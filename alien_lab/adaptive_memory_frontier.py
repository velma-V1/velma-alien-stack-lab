...
def _rule_fingerprint(rule_id: str, a: int, b: int, modulus: int, learned_level: int, evidence_id: str) -> str:
    return stable_hash({
        "rule_id": rule_id,
        "a": a,
        "b": b,
...
def generate_rules(seed: int, *, start_index: int, count: int, learned_level: int) -> tuple[MemoryRule, ...]:
    ...
    for index in range(start_index, start_index + count):
        ...
        rules.append(MemoryRule(
            ...
            learned_level=learned_level,
            evidence_id=evidence_id,
            fingerprint=_rule_fingerprint(rule_id, a, b, MODULUS, learned_level, evidence_id),
        ))
    return tuple(rules)
...
def build_level_packet(
    ...
    *,
    ...
    for position in range(1, task_count + 1):
        ...
        rng.shuffle(values)
        choices = {letter: value for letter, value in zip("ABCD", values)}
        ...
        tasks.append(task)
        sealed[task_id] = {"answer": answer, "final_value": correct_value, "deterministic_unique": True}
...
def _memory_rules_for_arm(tasks: tuple[MemoryTask, ...], store: MemoryStore, arm: str) -> tuple[MemoryRule, ...]:
    if arm == ARM_NONE:
        return ()
    if arm == ARM_FULL:
...
def render_memory_packet(tasks: tuple[MemoryTask, ...], store: MemoryStore, arm: str) -> tuple[str, tuple[str, ...]]:
...
def deterministic_preflight(seed: int) -> dict[str, Any]:
    ...
    checks.append({
        "name": "unique_sealed_answers",
        "passed": all(len(set(task.choices.values())) == 4 and sealed_a[task.task_id]["answer"] in task.choices for task in packet_a),
    })
    ...
    for level in range(1, 7):
        ...
        prior_rule_ok = prior_rule_ok and all(store.get(rule_id).learned_level < level for task in tasks for rule_id in task.rule_ids)
        promoted = promote_task_macros(store, tasks, {task.task_id: True for task in tasks}, learned_level=level)
        macro_evidence_ok = macro_evidence_ok and all(
            rule.evidence_id.startswith(f"memory-L{level}-") and rule.learned_level == level for rule in promoted
        )
    checks.append({
...
class AdaptiveMemoryRunner:
    def __init__(self, config: ExperimentConfig, *, client: Any, output_dir: Path | None = None, clock=time.monotonic) -> None:
        self.config = config
        self.client = client
    ...
    def _write_environment(self) -> None:
        metadata = self.client.model_metadata(self.config.model, timeout_seconds=5.0) if hasattr(self.client, "model_metadata") else None
        payload = {
    ...
    def _context_cap_record(self, *, level: int, arm: str, phase: str, model_seed: int, prompt_bytes: int, estimated_tokens: int, memory_ids: tuple[str, ...], store: MemoryStore, tasks: tuple[MemoryTask, ...]) -> dict[str, Any]:
        record = {
    ...
    def _run_arm(self, level: int, tasks: tuple[MemoryTask, ...], sealed: dict[str, dict[str, Any]], store: MemoryStore, arm: str, *, model_seed: int, phase: str) -> dict[str, Any]:
        prompt, memory_ids = render_memory_packet(tasks, store, arm)
        ...
        if estimated_tokens > self.config.context_limit:
            return self._context_cap_record(
                level=level, arm=arm, phase=phase, model_seed=model_seed,
                prompt_bytes=prompt_bytes, estimated_tokens=estimated_tokens,
                memory_ids=memory_ids, store=store, tasks=tasks,
            )
    ...
    def _promote_from_run(self, store: MemoryStore, tasks: tuple[MemoryTask, ...], run_record: dict[str, Any], *, learned_level: int) -> tuple[MemoryRule, ...]:
        success_by_task = {
            row["task_id"]: bool(row["verified_success"])
            for row in run_record.get("task_results", [])
            if row.get("verified_success") is not None
        }
        promoted = promote_task_macros(store, tasks, success_by_task, learned_level=learned_level)
        for rule in promoted:
            append_jsonl(self.output_dir / "memory.jsonl", {
    ...
    def _aggregate_accuracy(self, arm: str) -> float | None:
        rows = [
            ...
            if level["arms"].get(arm, {}).get("status") == "OK" and level["arms"][arm]["accuracy"] is not None
        ]
    ...
    def run(self) -> dict[str, Any]:
        ...
        raw_paths = [self.output_dir / "runs.jsonl", self.output_dir / "observations.jsonl", self.output_dir / "memory.jsonl"]
        if any(path.exists() for path in raw_paths):
            raise RuntimeError(f"output directory already contains raw Experiment 007 evidence: {self.output_dir}")
        ...
        if not preflight["passed"]:
            self.summary = {"interpretation": "PREFLIGHT_FAILED", "capability_valid": False, "preflight_passed": False, "levels_completed": 0}
        ...
        for level in range(1, MAX_LEVEL + 1):
            ...
            for arm in MEMORY_ARMS:
                arms[arm] = self._run_arm(level, tasks, sealed, store, arm, model_seed=model_seed, phase="frontier")
            if arms[ARM_FULL]["status"] == "CONTEXT_CAP_REACHED" and full_context_cap is None:
                full_context_cap = level
            if arms[ARM_FULL]["status"] == "OK" and arms[ARM_FULL]["accuracy"] is not None and arms[ARM_FULL]["accuracy"] < FRONTIER_PASS_THRESHOLD and full_first_failure is None:
            ...
            if primary_accuracy is not None and primary_accuracy >= FRONTIER_PASS_THRESHOLD:
                ...
                if len(promoted) < 7:
                    ...
                    break
                continue
            confirmation_tasks, confirmation_sealed = build_level_packet(self.config.seed, level, store, variant=1)
            ...
            if confirmation["status"] != "OK":
                ...
                break
            if self.confirmed_failure(primary_accuracy=primary_accuracy, confirmation_accuracy=confirmation["accuracy"]):
            ...
            promoted = self._promote_from_run(store, confirmation_tasks, confirmation, learned_level=level)
            level_row["promoted_macro_count"] = len(promoted)
            if len(promoted) < 7:
                capability_valid = False
                interpretation = "INVALID_MEMORY_PROMOTION"
